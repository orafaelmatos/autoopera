...
const ReportsView: React.FC<Props> = ({ appointments, services, onNavigateToPromotions }) => {
  ...
  return (
    <div className="space-y-8 sm:space-y-12 animate-fadeIn max-w-[1200px] mx-auto pb-20 px-4 sm:px-0">
      <header>
        <h2 className="text-2xl sm:text-4xl font-bold tracking-tight text-white">Relatórios <span className="text-gray-500">Analíticos</span></h2>
        <p className="text-sm sm:text-gray-500 mt-2 font-medium">Insights e inteligência baseada em dados reais.</p>
      </header>
      ...
      <div className="grid lg:grid-cols-2 gap-6 sm:gap-8">
        {/* Gráfico de Demanda */}
        <section className="bg-[#1c1c1e] border border-white/5 p-5 sm:p-8 rounded-[24px] sm:rounded-[32px] shadow-xl">
          ...
          <div className="flex items-end justify-between h-40 sm:h-56 gap-2 sm:gap-4">
            {daysData.map((d) => (
              <div key={d.day} className="flex-1 flex flex-col items-center gap-3 sm:gap-4 group">
                <div className="w-full relative flex items-end justify-center h-full bg-black/20 rounded-lg sm:rounded-2xl overflow-hidden border border-white/5">
                  <motion.div 
                    initial={{ height: 0 }}
                ...
                <span className="text-[8px] sm:text-[10px] font-black text-gray-600 uppercase tracking-widest">{d.day}</span>
              </div>
            ))}
          </div>
        </section>
          ...
          <div className="space-y-6 sm:space-y-8">
            {platforms.map(p => (
              <div key={p.name} className="space-y-3">
                <div className="flex justify-between items-center text-[8px] sm:text-[10px] font-black uppercase tracking-[0.1em]">
                  <span className="flex items-center gap-2 text-gray-400">{p.icon} {p.name}</span>
                  <span className="text-white">{p.value}%</span>
                </div>
                <div className="w-full bg-black/40 h-2 sm:h-2.5 rounded-full overflow-hidden border border-white/5 p-[2px]">
                  <motion.div 
                    initial={{ width: 0 }}
        ...
        {/* Ranking de Serviços */}
        <section className="bg-[#1c1c1e] border border-white/5 p-5 sm:p-8 rounded-[24px] sm:rounded-[32px] shadow-xl lg:col-span-2">
            ...
            {services.slice(0, 6).map((s, idx) => (
              <div key={s.id} className="bg-black/20 p-4 sm:p-5 rounded-[20px] sm:rounded-[24px] border border-white/5 flex items-center justify-between hover:bg-black/40 transition-all group">
                ...
                <div className="text-right">
                  <div className="flex items-center gap-1 text-green-500 font-bold text-xs sm:text-sm tracking-tight justify-end">
                    <ArrowUpRight size={12} className="sm:w-[14px] sm:h-[14px]" />
                    R$ {(s.price * (1 - s.commission/100) * 12).toFixed(0)}
                  </div>
                  <p className="text-[8px] text-gray-700 font-black uppercase tracking-[0.1em] mt-0.5">EST. MENSAL</p>
                </div>
              </div>
            ))}
          </div>

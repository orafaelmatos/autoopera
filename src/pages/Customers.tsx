    ...
    <div className="space-y-8 sm:space-y-12 animate-fadeIn max-w-[1200px] mx-auto pb-20">
      <header className="flex flex-col md:flex-row md:items-end justify-between gap-4 sm:gap-6">
        ...
        <button 
          ...
          className="bg-accent text-white px-5 sm:px-6 py-3.5 sm:py-3 rounded-2xl text-[11px] sm:text-[13px] font-bold uppercase tracking-widest flex items-center justify-center gap-2 hover:bg-[#0063CC] transition-all shadow-xl active:scale-95 w-full md:w-auto"
        >
          <UserPlus size={18} />
      ...
      <div className="relative group mx-1">
        <Search className="absolute left-5 sm:left-6 top-1/2 -translate-y-1/2 text-gray-500 group-focus-within:text-white transition-colors" size={18} />
        <input 
          type="text" 
          placeholder="Buscar cliente..."
          className="w-full bg-[#1c1c1e] border border-white/5 rounded-[20px] sm:rounded-[24px] pl-12 sm:pl-16 pr-6 sm:pr-8 py-4 sm:py-5 text-white focus:border-white/20 focus:outline-none transition-all placeholder:text-gray-600 text-base sm:text-lg"
          value={searchTerm}
        ...
        {filtered.length > 0 ? filtered.map(customer => (
          <div key={customer.id} className="bg-[#1c1c1e] border border-white/5 p-4 sm:p-6 rounded-[24px] sm:rounded-[32px] flex flex-col md:flex-row md:items-center justify-between gap-4 sm:gap-6 hover:bg-[#2c2c2e] transition-all group">
            <div className="flex items-center gap-4 sm:gap-6">
              <div className="w-14 h-14 sm:w-16 sm:h-16 rounded-[20px] sm:rounded-[24px] bg-black/40 flex items-center justify-center text-white/60 border border-white/5 group-hover:scale-105 transition-transform overflow-hidden">
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 sm:gap-3">
                  ...
                  <div className="flex items-center gap-1 bg-white/5 px-2 sm:px-2.5 py-0.5 sm:py-1 rounded-full text-white/40 text-[8px] sm:text-[10px] font-bold uppercase tracking-wider border border-white/5 whitespace-nowrap">
            ...
            <div className="flex items-center gap-2 sm:gap-3">
              ...
              <button 
                ...
                className="flex-[2] md:flex-none px-5 sm:px-6 py-3.5 sm:py-4 bg-[#1c1c1e] border border-white/5 hover:border-white/20 text-white rounded-xl sm:rounded-[20px] text-[11px] sm:text-[13px] font-bold uppercase tracking-widest transition-all active:scale-95 shadow-lg"
              >
                Detalhes
              </button>
            </div>
          ...
          <div className="fixed inset-0 bg-black/80 backdrop-blur-xl z-[100] flex items-center justify-center p-4">
            ...
            >
              ...
              <div className="overflow-y-auto custom-scrollbar">
                <div className="h-32 md:h-40 bg-gradient-to-br from-accent to-[#004BB3] relative overflow-hidden flex-shrink-0">
                  <div className="absolute inset-0 bg-black/10"></div>
                  <div className="absolute -bottom-8 md:-bottom-12 left-6 md:left-10">
                    <div className="w-20 h-20 md:w-24 md:h-24 rounded-[28px] md:rounded-[32px] bg-[#1c1c1e] border-[4px] md:border-[6px] border-[#1c1c1e] flex items-center justify-center text-accent text-3xl md:text-4xl font-bold shadow-2xl overflow-hidden">
                ...
                <div className="pt-12 md:pt-16 p-6 md:p-10">
                    ...
                    <div className="w-full">
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-4 mt-2">
                        <p className="text-gray-500 text-sm font-medium flex items-center gap-2">
                          <Phone size={14} className="text-accent" /> {selectedCustomer.phone}
                        </p>
                        <div className="hidden sm:block w-1 h-1 rounded-full bg-white/10"></div>
                        <p className="text-gray-500 text-sm font-medium flex items-center gap-2">
                          <Calendar size={14} className="text-accent" /> Visto em: {selectedCustomer.lastVisit}
                        </p>
                      </div>
                    </div>
                    <div className="w-full lg:w-auto bg-accent/10 p-5 md:p-6 rounded-[24px] md:rounded-[28px] border border-accent/20 flex-shrink-0">
                      <p className="text-[10px] text-accent font-bold uppercase tracking-[0.2em] mb-2 leading-none">Pontos Acumulados</p>
                      <div className="flex items-end gap-2">
                        <span className="text-3xl md:text-4xl font-bold text-accent leading-none">{selectedCustomer.points}</span>
                        <span className="text-sm font-bold text-accent/60 uppercase tracking-widest mb-1">pts</span>
                      </div>
                    </div>
                  </div>
                      ...
                      <div className="bg-black/20 border border-white/5 p-5 md:p-6 rounded-[24px]">
                        <div className="flex items-center justify-between mb-4">
                          <h4 className="font-bold flex items-center gap-2 text-xs md:text-sm text-gray-400 uppercase tracking-wider">
                            <MessageSquare size={16} className="text-accent" /> Perfil Técnico
                          </h4>
                          {!isEditingNotes ? (
                            <button onClick={() => setIsEditingNotes(true)} className="text-[10px] text-accent uppercase font-bold tracking-[0.15em] hover:text-[#0063CC] transition-colors py-1 px-3 bg-accent/5 rounded-lg border border-accent/10">Editar</button>
                          ) : (
                        ...
                        {isEditingNotes ? (
                          <div className="space-y-4">
                            <div className="space-y-1.5">
                              ...
                              <textarea 
                                className="w-full bg-black/40 border border-white/5 rounded-2xl p-4 text-sm text-white focus:outline-none focus:border-accent/50 transition-all placeholder:text-gray-700 min-h-[100px] font-medium"
                                value={tempNotes}
                  ...
                  <div className="space-y-6">
                    <h4 className="font-bold flex items-center gap-2 text-sm text-gray-300 uppercase tracking-widest">
                      <Ticket size={16} className="text-accent" /> Prêmios Disponíveis
                    </h4>
                        ...
                        <button 
                          ...
                          className={`w-full flex items-center justify-between p-4 rounded-2xl border transition-all group ${
                            selectedCustomer.points >= reward.cost 
                              ? 'bg-[#1c1c1e] border-white/5 hover:border-accent/50 hover:bg-[#2c2c2e]' 
                              : 'bg-black/20 border-white/5 opacity-40 grayscale cursor-not-allowed'
                          }`}
                        >
                          <div className="flex items-center gap-4">
                            <div className={`p-2 rounded-xl ${selectedCustomer.points >= reward.cost ? 'bg-accent/10 text-accent' : 'bg-gray-800 text-gray-500'}`}>
                              <reward.icon size={18} />
                            </div>
                          ...
                          {selectedCustomer.points >= reward.cost && <ChevronRight size={18} className="text-white/20 group-hover:text-accent group-hover:translate-x-1 transition-all" />}
                        </button>
                      ))}
                    </div>
                    <button 
            ...
            >
              ...
              <div className="space-y-5">
                <div className="space-y-2">
                  ...
                  <input type="text" placeholder="Ex: João da Silva" value={newName} onChange={e => setNewName(e.target.value)} className="w-full bg-black/40 border border-white/5 rounded-2xl px-5 py-3.5 sm:py-4 text-white focus:border-accent/50 outline-none transition-all placeholder:text-gray-700" />
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-bold text-gray-500 uppercase tracking-widest ml-2">WhatsApp / Telefone</label>
                  <input type="text" placeholder="(00) 00000-0000" value={newPhone} onChange={e => setNewPhone(e.target.value)} className="w-full bg-black/40 border border-white/5 rounded-2xl px-5 py-3.5 sm:py-4 text-white focus:border-accent/50 outline-none transition-all placeholder:text-gray-700" />
                </div>
                <div className="space-y-2">
                  <label className="text-[10px] font-bold text-gray-500 uppercase tracking-widest ml-2">Observações Iniciais</label>
                  <textarea placeholder="Preferências, endereço ou detalhes extras..." value={newNotes} onChange={e => setNewNotes(e.target.value)} className="w-full bg-black/40 border border-white/5 rounded-2xl px-5 py-3.5 sm:py-4 text-white focus:border-accent/50 outline-none resize-none transition-all placeholder:text-gray-700" rows={3} />
                </div>
              </div>
